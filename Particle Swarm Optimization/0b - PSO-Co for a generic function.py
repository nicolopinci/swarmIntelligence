...
class Agent:
    ...
    def computeVelocity(self, c1, c2, agents, locality):
        xi = self.computeXi(c1, c2)
        self.vel = xi*(np.array(self.vel) + c1*random()*np.array(self.distances(self.bestPos)) + c2*random()*np.array(self.distances(self.bestLocalPosition(agents, locality))))

    def computePosition(self, c1, c2, agents, locality):
        self.computeVelocity(c1, c2, agents, locality)
    ...
    def computeXi(self, c1, c2):
        phi = c1 + c2
        return 2.0/abs(2- phi - math.sqrt(phi*phi - 4*phi))
...
while(samePosition == False):
    ...
    for a in range(0, len(agents)):
        agents[a].computePosition(2.3, 1.8, agents, locality)
