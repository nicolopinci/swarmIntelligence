...
class Agent:
    ...
    def align(self, agents, centerRotation, delta):
        cr = centerRotation.dim
        radius = self.distance(centerRotation)
        self.changeAcceleration(centerRotation.acc, 1)
        
        newPosition = []
        
        for i in range(0, len(centerRotation.dim)):
            newPosition.append(self.dim[i] - delta[i])
            
        newAgent = Agent(newPosition, [0, 0, 0], [0, 0, 0], 'tempTarget')

        self.moveTo(newAgent)
...
while(samePosition == False):
    ...
    for agent in agents:
        agent.separate(agents, separation, locality)
        agent.cohere(agents, cohesion)
        agent.align(agents, centerRotation, delta)        
